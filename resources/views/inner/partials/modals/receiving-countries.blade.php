<div class="modal fade modalbox" wire:ignore.self id="rc_receiving_countries" tabindex="-1" role="dialog">
        ...
        <div class="modal-content pt-0 bg-capsule">
            ...
            <div class="modal-body p-0">
                <div class="bg-dark-blue   justify-content-between d-flex  align-items-center p-2">
                    <h3 class="text-white modal-title">{{ $rc_title }}</h3>
                    <a href="#" class="ms-2" data-bs-dismiss="modal"  data-dismiss="modal">
                        <img src="{{ asset('icons/close-modal.png') }}" style="width: 30px;" alt="">
                    </a>
                </div>
                <div class="p-1">
                    <div class="form-group  searchbox not-empty">
                        <input type="text" wire:model.debounce.500ms="rc_search_query" class="form-control"
                               placeholder="Search...">
                        <i class="input-icon">
                ...
                @if(!empty($rc_data))
                    <ul class="listview image-listview inset">
                        @foreach($rc_data as $rc)
                            <li>
                                <a href="#" class="item"  data-bs-dismiss="modal" wire:click.prevent="rcSelection('{{ json_encode($rc) }}')">
{{--                                    <img src="{{ url('images/flags') }}/{{ strtolower($rc['iso2']) }}.svg" alt="image"--}}
{{--                                         class="image">--}}
                                    <div class="in">
                                        <div>{{ $rc['name'] }}</div>
                                        <span>{{ $rc['currency'] }}</span>
                                    </div>
                                </a>
                            </li>
                        @endforeach
