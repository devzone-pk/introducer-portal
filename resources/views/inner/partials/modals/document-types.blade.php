{{--done--}}
<div class="modal fade modalbox" wire:ignore.self id="document-types-modal" tabindex="-1" role="dialog">
        ...
        <div class="modal-content pt-0 bg-capsule">
            ...
            <div class="modal-body p-0 ">
                ...
                <div class="bg-dark-blue  d-flex  justify-content-between align-items-center p-2">
                    <h3 class="text-white modal-title">{{ $dt_title }}</h3>
                ...
                <div class="p-1">
                    <div class="form-group  searchbox not-empty">
                        <input type="text" wire:model.debounce.500ms="dt_search_query" class="form-control"
                               placeholder="Search...">
                        <i class="input-icon">
                ...
                @if(!empty($dt_data))
                    <ul class="listview image-listview remove-arrow inset">
                        @foreach($dt_data as $rc)
                            <li>
                                <a href="#" class="item"  data-bs-dismiss="modal" wire:click.prevent="dtSelection('{{ json_encode($rc) }}')">

                                    <div class="in">
                                        <div>{{ $rc['name'] }}<br/>
                                            <span class="text-muted">{{ ucwords($rc['additional_info']) }}</span>
                                        </div>
